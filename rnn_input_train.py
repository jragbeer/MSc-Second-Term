...
# training loop
for x, y_, epoch in txt.rnn_minibatch_sequencer(code_text, BATCHSIZE, SEQLEN, nb_epochs=10):
    ...
    if step % _50_BATCHES == 0 and len(validation_text) > 0:
        ...
        bsize = len(validation_text) // VALI_SEQLEN
        txt.print_validation_header(len(code_text), bookranges)
        vali_x, vali_y, _ = next(txt.rnn_minibatch_sequencer(validation_text, bsize, VALI_SEQLEN, 1))  # all data in 1 batch
    ...
    if step // 10 % _50_BATCHES == 0:
        saved_file = saver.save(sess, 'checkpoints3/rnn_train_' + timestamp, global_step=step)
